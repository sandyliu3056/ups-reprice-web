...
|---|---|
| 📁 匯入檔案 | 選帳單、產生報表、匯出對帳單 |
| 💲 附加費設定 | 燃油(可按渠道設不同期間,日期用內建月曆挑)、28 條渠道的附加費率、申報價值級距。選到哪條渠道就顯示那條的 Zone,匯出的模板一致 |
| 📏 尺寸規則 | DIM 除數、AHS/LPS 十二個門檻、批量設定渠道 |
| 🚚 渠道 | 新增渠道、基本運費表匯出匯入(可一次選多個檔)、檢視已載入的運費、全部渠道一覽 |
| ⚡ Demand | 四個子分頁,已接進計價 |
...
金額從帳單的**申報金額欄**讀,不是 UPS 的收費金額。兩者差很多:同一行申報
647.15、收費 5.95。拿收費去查級距會落在免收上限以下,結果永遠算 0。

欄位預設是 AW(第 48 欄),可在設定的 `dv_column` 改成別的字母,和桌面版
同一個換算(`A`→0、`AW`→48)。有標題列的檔案以標題為準。

級距公式照桌面版:免收上限以下不收;超過之後每一個級距單位(含第一個,
不足一個算一個)收一次費率,不低於最低收費。
...
- **匯入過去帳單**會自動歸檔,不用另外按儲存。
- **產生報表時,這一期會先歸檔再計價** —— 順序照桌面版。理由是計價失敗的那
  一期如果沒進歷史,下個月的跨期更正就沒有東西可以對照。
- **同一個期號再進來就整期取代,不是疊上去。** 同一份檔案匯兩次不該把每一列
  變成兩列。狀態列會分別寫「已存入」和「已更新」。
- 索引存在瀏覽器裡,重開還在。

「已匯入的帳單」視窗上方有一條瀏覽器空間量表:已用多少、剩多少、上限約多少,
下面列出四項各占多少(費率設定、追蹤號對照、帳單索引、操作紀錄),並換算成
「以現在的用量還放得下約幾期」。上限標「約 5 MB」是因為 localStorage 沒有
標準的查詢介面,5 MB 是瀏覽器一般給每個網站的額度。

37 期實測:追蹤號對照 3.67 MB、帳單索引 7.6 KB,合計約 3.71 MB,還能再放
十幾期。占空間的是追蹤號對照(七萬七千筆),不是帳單索引。

「已匯入的帳單」視窗讀的是這份索引,不是這次載進來的那幾列,所以重開瀏覽器
之後清單還在,並且多一欄歸檔時間。

明細列(查追蹤號用的那張表)只留在這次工作階段裡 —— 55 萬列存不進瀏覽器,
要查舊期的明細就重新匯入那幾個檔案。

### 查帳單歷史

追蹤號或發票期號都能查。整期六千多列一次塞進畫面會卡住,所以表格分頁,
一頁四百列,用 « ‹ › » 翻,旁邊寫著「第幾頁 / 共幾頁」和目前的列區間 ——
每一列都看得到,只是一次畫一頁。換查詢字串會回到第一頁。
