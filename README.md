...
運費之外,AHS 與 LPS 也是差額。照桌面版 `_pure_ca` 那一段:

- **一件只收一個 AHS**,Weight / Dimension / Packaging 三種取費率最高的那一個。
- **AHS 與 LPS 互斥**。A 觸發 LPS 時,它的 AHS 貢獻是 0。
- C 是大包裹 → 收 LPS 差額,並退掉 A 的 AHS(C 沒有 AHS)。
- A 是大包裹但 C 不是 → 退掉 A 的 LPS,這是「從大包裹降級成 AHS」。
- 是不是大包裹,看**我們自己的門檻**判 C 和 A,不是看 UPS 開了什麼代碼 ——
  UPS 開 LPS 有可能只是在沖掉 A 的那一筆。
- 這一筆的 ADJ 上完全沒有 AHS / LPS 代碼時整段不做,尺寸驅動的欄位留 0。

A 側的渠道用原出貨的;跨期單本期沒有原出貨,只能查帳單歷史。兩側的 Zone 目前
共用這一筆的 Zone。
