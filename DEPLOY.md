...
版本日期**不印在畫面上**(平常沒人要看它,佔標題列的位置)。要問的時候有兩個地方:

- **滑鼠停在標題列的「DEVELOPED BY SANDY LIU」上**,會浮出:
  `版本(這份 index.html 的最後修改時間):2026-08-28 17:02`
- **開瀏覽器主控台**(F12 → Console),開頁時會印一行:
  `UPS Reprice — build 2026-08-28 17:02`

那是伺服器回報的 `index.html` 最後修改時間,不必手動維護。**日期沒有跟著這次部署往前走,就是沒有拿到新檔**,不必再一個一個功能去找哪裡不見了。
...
Admin 分頁上半部是帳號清單:新增帳號(**帳號** + 名稱 + Email(選填) + 密碼 + 角色)、改角色、改密碼、刪除。新帳號的費率是**空白**的,由他自己匯入。

登入用的是**帳號**,不是 email。Supabase 的密碼登入一定要一個 email,所以帳號會補成 `<帳號>@ups-reprice.invalid` 去驗證 —— `.invalid` 是 RFC 2606 保留的網域,永遠解析不到,寄不出信也不會誤寄給別人。Email 欄只是聯絡資訊,選填。最早用真 email 開的主帳號照舊用 email 登入(輸入裡有 @ 就原樣送出)。

如果你的 Supabase 拒絕這個網域,把 `index.html` 的 `ACCOUNT_DOMAIN` 和 Edge Function 的 `DOMAIN` 改成同一個你要用的字串就好。

新增與刪除需要 service_role 金鑰,那把金鑰在 Edge Function 裡,不在瀏覽器。函式每次都先驗來人是不是主帳號,驗不過直接回絕。

## 帳單歷史也是一個帳號一份

明細(IndexedDB)和跨期對照表都綁 user id,同一台機器換人登入看不到前一個人的帳單。

分帳號**之前**存的那一份,只有**主帳號**登入時會自動收進來,一次就好:主帳號自己還沒有任何歷史、而舊的那份有東西時就搬過去,搬完把舊的清空。不會問、畫面上也沒有提示。子帳號登入時完全不碰它 —— 一個子帳號在別人的機器上登入一次就把整台機器的帳單吸進自己名下,那正是分帳號要避免的事。

主帳號也讀不到別的帳號的歷史。刪掉一個帳號時,他存在這台機器上的明細會一起刪掉(只刪不讀)。

新機器或新帳號的歷史是空的,跨期更正要先把前幾期帳單跑過一次。
