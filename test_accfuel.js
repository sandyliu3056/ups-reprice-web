...
JSDOM.fromFile(path.join(__dirname, "index.html"), {
  ...
  run(`window.__t = {
    seed(){
      ...
      /* 基本運費表。沒有它整筆會被「這條渠道沒有費率」擋掉、附加費一起
         歸零(index.html:2322 的 noRates),測不到燃油底。 */
      CFG.raw.base_rate_tables=[{
        service:"Ground Residential", residential:"true",
        columns:["Weight","Zone 8"],
        rows:[[5,10.00]]
      }];
